...
optromloader18: optromloader.asm
	@echo "*** assembling $@..."
	$(fasm) $(fasm_extraopts) -d build_date=$(date) -d sectors_per_track=18 optromloader.asm $@
optromloader15: optromloader.asm
	@echo "*** assembling $@..."
	$(fasm) $(fasm_extraopts) -d build_date=$(date) -d sectors_per_track=15 optromloader.asm $@
optromloader9: optromloader.asm
	@echo "*** assembling $@..."
	$(fasm) $(fasm_extraopts) -d build_date=$(date) -d sectors_per_track=9 optromloader.asm $@
fd1440.img: optromloader.asm optrom.bin
	@echo "*** assembling $@..."
	$(fasm) $(fasm_extraopts) -d build_date=$(date) -d sectors_per_track=18 -d pad_to_bytes=1474560 -d include_optrom="'optrom.bin'" optromloader.asm $@
fd720.img: optromloader.asm optrom.bin
	@echo "*** assembling $@..."
	$(fasm) $(fasm_extraopts) -d build_date=$(date) -d sectors_per_track=9 -d pad_to_bytes=737280 -d include_optrom="'optrom.bin'" optromloader.asm $@
fd1200.img: optromloader.asm optrom.bin
	@echo "*** assembling $@..."
	$(fasm) $(fasm_extraopts) -d build_date=$(date) -d sectors_per_track=15 -d pad_to_bytes=1228800 -d include_optrom="'optrom.bin'" optromloader.asm $@
fd360.img: optromloader.asm optrom.bin
	@echo "*** assembling $@..."
	$(fasm) $(fasm_extraopts) -d build_date=$(date) -d sectors_per_track=9 -d pad_to_bytes=368640 -d include_optrom="'optrom.bin'" optromloader.asm $@
